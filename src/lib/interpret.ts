...
export function buildMorningPrompt(cards: DrawnCards): string {
  const resolved = SPREAD_ORDER.map((arcana) => ({ card: getCardById(cards[arcana]), pos: SPREAD_POSITIONS[arcana] }));
  const [first, second, third] = resolved;

  return `당신은 30년 경력의 타로 리더입니다. 오늘 뽑은 아래 3장을 당신의 타로 지식으로 깊이 있게 해석해, 오늘 하루를 위한 한국어 리딩을 작성하세요.

[오늘 뽑은 카드 — 쓰리 카드 일일 명상법(100일), 분리 배열법 응용]
${todayCardsSummary(cards)}

[출력 구조]
아래 네 개의 제목을 대괄호까지 그대로 쓰고, 순서도 그대로 지키세요. 각 제목 아래에 본문을 쓰세요.

[${first?.pos.title ?? "1번 자리"}] ${first?.card?.nameKo ?? ""}
- 이 카드의 그림과 상징에서 출발해, 이 카드가 오늘 하루에 깔아놓는 바탕이 무엇인지 5~7문장으로 쓰세요.
- 이 카드 고유의 요소(등장인물, 사물, 색, 자세, 배경)를 최소 두 가지 이상 짚고, 그것이 왜 그런 의미로 읽히는지 설명하세요.
- 이 자리의 질문("${first?.pos.question ?? ""}")에 분명히 답하세요.

[${second?.pos.title ?? "2번 자리"}] ${second?.card?.nameKo ?? ""}
- 1번의 바탕을 오늘 내가 '어떤 사람으로' 살아내게 되는지를 5~7문장으로 쓰세요.
- 이 인물의 수트(원소)와 등급(시종·기사·여왕·왕)이 말하는 태도의 결을 구체적으로 풀어주세요.
- 이 태도가 과해질 때와 모자랄 때가 오늘 어떤 모습으로 나타나는지도 한 번씩 짚어주세요.

[${third?.pos.title ?? "3번 자리"}] ${third?.card?.nameKo ?? ""}
- 앞의 둘이 오늘 현실에서 어떤 장면·사건·행동으로 떨어지는지를 5~7문장으로 쓰세요.
- 이 카드의 숫자와 수트가 가리키는 국면을 설명하고, 오늘 실제로 벌어질 법한 구체적인 상황을 두세 가지 예로 드세요.
- 이 국면에서 조심할 지점을 반드시 하나 짚으세요.

[오늘 하루로 엮기]
- 세 장을 하나의 문장 흐름으로 이어, 오늘이 어떤 하루가 될 수 있는지 4~6문장으로 정리하세요.
- 마지막에 "오늘 해볼 것: "으로 시작하는 줄을 하나 붙이고, 오늘 안에 실제로 해볼 수 있는 아주 구체적인 행동 하나만 제안하세요.

[반드시 지킬 것]
1. 깊이 있게 쓰세요. 카드 이름만 바꾸면 아무 카드에나 들어맞는 일반론(예: "균형을 찾으세요", "내면의 목소리를 들으세요")은 쓰지 마세요. 반드시 이 카드에서만 나올 수 있는 이야기를 하세요.
2. 아직 하루가 시작되지 않았습니다. 하루를 평가하거나 위로하지 마세요. "좋은 하루가 될 거예요" 같은 덕담도 넣지 마세요.
3. 사용자의 감정이나 처지를 넘겨짚지 마세요. "지금 지쳐 있다면", "마음이 무겁겠지만" 같은 표현을 쓰지 마세요.
4. 오늘 뽑지 않은 다른 카드나 기간(이번 주, 이번 달, 내일 등)은 언급하지 마세요.
5. 역방향은 이 타로 방식에서 쓰지 않습니다. 모두 정방향으로만 해석하세요.
6. 각 항목을 불릿(-)으로 나열하지 말고, 제목 아래에 이어지는 문단으로 쓰세요.`;
}
...
(8~12문장)
- 세 장을 한 장씩 순서대로 짚으면서, 그 카드가 그 자리에서 가리킨 의미와 사용자가 쓴 성찰을 직접 맞춰보세요.
- 각 카드마다 최소 두 문장씩은 쓰세요. 그 카드의 상징에서 나온 구체적인 이야기여야 합니다.
- 사용자가 실제로 쓴 표현을 그대로 인용하며 근거로 삼으세요.
